...
当进程发出信号的时候会调用`kill`系统调用，这个时候内核处理`kill`时会讲对应进程的信号置位，信号量的维护放在TCB里，在Gcore里它用一个叫`SigInfo`的数据结构维护。
...
1. 将进程的`sigmask`添加`set`的标志位
2. 将进程的`sigmask`删去`set`的标志位
3. 将进程的`sigmask`赋值为`set`
...
自定义函数的加载逻辑就显得有点麻烦了，我们设计了一个单独的trampoline用于signal的自定义处理函数.这里留一个问题,一会儿揭晓答案: 为什么不能沿用原来的Trampoline, 非要设计出单独的trampoline呢? 我们先继续看下去.
